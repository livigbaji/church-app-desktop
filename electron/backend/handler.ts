...
export function Handler(event: string, useInvokeEvent = false) {
  return (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
    ...
    ipcMain.handle(event, async (_event: IpcMainInvokeEvent, ...args) => {
        ...
        for (const { index, constructor } of requiredParameters) {
          try {
            await validateOrReject(
              plainToInstance(constructor, args[index] || {})
            );
          } catch (e: unknown) {
            return {
              success: false,
              error: formatError(e as ValidationError[]),
            };
          }
        }
      }
      ...
      return method
        .apply(target, extraArgs.concat(args))
        .then((data: unknown) => {
...
export function Validate<T>(constructor: T) {
  return (
    ...
    let existingRequiredParameters: { index: number; constructor: T }[] =
      Reflect.getOwnMetadata(validateMetadataKey, target, propertyKey) || [];
...
const formatError = (errors: ValidationError[]) => {
  ...
  return errors.reduce(
    (acc: errorType, error: ValidationError) => ({
      ...acc,
